...
import { deriveEtheriumWallet } from "@/app/lib/serveractions/EtherWallet";
...
export async function POST(req) {
  try {
    ...
    if (wallet_type === "solana") {
      const solana_wallet_number = user_data.walletCounter.solana;
      const derivation_path = `m/44'/501'/${solana_wallet_number}'/0'`;
      ...
      const ethereum_wallet_number = user_data.walletCounter.ethereum;
      const derivation_path = `m/44'/60'/${ethereum_wallet_number}'/0'`;
      console.log(derivation_path);
      const result = await deriveEtheriumWallet(seed, derivation_path);
      new_wallet.privateKey = result.privateKey;
      new_wallet.publicKey = result.publicKey;
    } else {
      return NextResponse.json(
